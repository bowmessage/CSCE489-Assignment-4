...
def decode_main(filename):
	...
	code_begin_addr = 4096/4
	binhex_code = binhex[code_begin_addr*2:code_begin_addr*2+0xffffffff]
	if m is None:
		binhex_code = binhex
	xed = pyxed.Decoder()
	xed.set_mode(pyxed.XED_MACHINE_MODE_LEGACY_32, pyxed.XED_ADDRESS_WIDTH_32b)
	hexdig = binhex_code
	q = open('instructions.txt', 'w')
	q.write(hexdig)
	q.close()
	...
	while True:
		try:
			...
			if(q == len(hexdig)):
				break
			xed.runtime_address = 0x00000000 + q/2 + int(image_base, 16) + code_begin_addr + 3072
